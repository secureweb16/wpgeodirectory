...
<div class="inner_content_tab_main gd-import-export">
	<div class="gd-content-heading">
		...
		?>
		<div id="gd_ie_imneighbourhoods" class="metabox-holder accordion">
			<div class="card p-0 mw-100 border-0 shadow-sm" style="overflow: initial;">
				...
				<div id="gd_ie_im_neighbourhoods" class="gd-hndle-pbox card-body">
				<?php
					$settings = array(
						array(
							'name'       => __( 'If neighbourhood_id/slug exists', 'geodirlocation' ),
							'desc'       => __( 'If the neighbourhood_id/neighbourhood_slug column exists in the CSV, you can either update the neighbourhood or it can be skipped.', 'geodirlocation' ),
							'id'         => 'gd_im_choiceneighbourhood',
							'default'    => 'skip',
							'type'       => 'select',
							'options' => array(
								'skip' => __( 'Skip row', 'geodirlocation'),
								'update' => __( 'Update Neighbourhood', 'geodirlocation'),
					...
					?>
					<div data-argument="gd_im_neighbourhood_file" class="form-group row">
						<label for="gd_im_neighbourhood_file" class="font-weight-bold col-sm-3 col-form-label"><?php _e( 'Upload CSV file', 'geodirlocation' ); ?></label>
						<div class="col-sm-9">
							<?php
							echo aui()->button(
								array(
									...
									'onclick'   => "jQuery('#gd_im_neighbourhoodplupload-browse-button').trigger('click');"
								)
							);
					...
					<div class="gd-imex-box">
						<div class="plupload-upload-uic hide-if-no-js" id="gd_im_neighbourhoodplupload-upload-ui">
							<input type="hidden" readonly="readonly" name="gd_im_neighbourhood_file" class="gd-imex-file gd_im_neighbourhood_file" id="gd_im_neighbourhood" onclick="jQuery('#gd_im_neighbourhoodplupload-browse-button').trigger('click');" />
							<input id="gd_im_neighbourhoodplupload-browse-button" type="hidden" value="<?php esc_attr_e( 'Select & Upload CSV', 'geodirlocation' ); ?>" class="gd-imex-cupload button-primary" />
						...
						<span id="gd_im_neighbourhoodupload-error" class="alert alert-danger" style="display:none"></span>
						...
						<div class="gd-imex-btns" style="display:none;">
							...
							<input onclick="geodir_location_prepare_import(this, 'neighbourhood')" type="button" value="<?php esc_attr_e( 'Import data now', 'geodirlocation' ); ?>" id="gd_import_data" class="btn btn-primary" />
							<input onclick="geodir_location_resume_import(this, 'neighbourhood')" type="button" value="<?php _e( "Continue Import Data", 'geodirlocation' ); ?>" id="gd_continue_data" class="btn btn-primary" style="display:none"/>
							<input type="button" value="<?php _e("Terminate Import Data", 'geodirlocation');?>" id="gd_stop_import" class="btn btn-danger" name="gd_stop_import" style="display:none" onclick="geodir_location_terminate_import(this, 'neighbourhood')"/>
		...
		<div id="gd_ie_exneighbourhoods" class="metabox-holder accordion">
			<div class="card p-0 mw-100 border-0 shadow-sm" style="overflow: initial;">
				<div class="card-header bg-white rounded-top">
					<h2	class="gd-settings-title h5 mb-0 "><?php echo __( 'Neighbourhoods: Export CSV', 'geodirlocation' ); ?></h2>
				</div>
				<div id="gd_ie_ex_neighbourhoods" class=" gd-hndle-pbox card-body">
					<?php
					echo aui()->select(
						array(
							'label_col'  => '3',
					...
					<div class="gd-ie-actions d-flex flex-row align-items-center">
						<input data-export="neighbourhoods" type="submit" value="<?php echo esc_attr( __( 'Export CSV', 'geodirlocation' ) );?>" class="btn btn-primary" name="gd_start_export" id="gd_start_export">
