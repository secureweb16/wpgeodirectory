...
foreach ( $post_types as $post_type => $row ) {
	...
	$taxonomy_options[ $post_type ] = wp_sprintf( __( '%s Categories', 'geodirlocation' ), $cpt_name ) . ' (' . $terms_count . ')';
}

if ( ! empty( $taxonomy_options ) ) {
	$taxonomy_options = array_merge( array( '' => __( 'All', 'geodirlocation' ) . ' (' . $total_terms . ')' ), $taxonomy_options );
}
...
<div class="inner_content_tab_main gd-import-export">
	<div class="gd-content-heading">
		...
		?>
		<div id="gd_ie_imcat_locations" class="metabox-holder accordion">
			<div class="card p-0 mw-100 border-0 shadow-sm" style="overflow: initial;">
				<div class="card-header bg-white rounded-top">
					<h2	class="gd-settings-title h5 mb-0 "><?php echo __( 'Category + Locations Description: Import CSV', 'geodirlocation' ); ?></h2>
				</div>
				<div id="gd_ie_im_cat_locations" class="gd-hndle-pbox card-body">
					<div data-argument="gd_im_cat_location_file" class="form-group row">
						<label for="gd_im_cat_location_file" class="font-weight-bold col-sm-3 col-form-label"><?php _e( 'Upload CSV file', 'geodirlocation' ); ?></label>
						<div class="col-sm-9">
							<?php
							echo aui()->button(
								array(
									...
									'onclick'   => "jQuery('#gd_im_cat_locationplupload-browse-button').trigger('click');"
								)
							);
							?>
						</div>
					...
					<div class="form-group row">
						<div class="col-sm-12"><?php _e( 'Export csv from GD Locations  + Categories Descriptions and update descriptions in exported csv file then import csv here. Location description updated to matching term_id & location slugs.', 'geodirlocation' ); ?></div>
					</div>
					<div class="gd-imex-box">
						<div class="plupload-upload-uic hide-if-no-js" id="gd_im_cat_locationplupload-upload-ui">
							<input type="hidden" readonly="readonly" name="gd_im_cat_location_file" class="gd-imex-file gd_im_cat_location_file" id="gd_im_cat_location" onclick="jQuery('#gd_im_cat_locationplupload-browse-button').trigger('click');" />
							<input id="gd_im_cat_locationplupload-browse-button" type="hidden" value="<?php esc_attr_e( 'Select & Upload CSV', 'geodirlocation' ); ?>" class="gd-imex-cupload button-primary" />
							<input type="hidden" id="gd_im_cat_location_allowed_types" data-exts=".csv" value="csv" />
						...
						<div class="gd-imex-btns" style="display:none;">
							...
							<input onclick="geodir_location_prepare_import(this, 'cat_location')" type="button" value="<?php esc_attr_e( 'Import data now', 'geodirlocation' ); ?>" id="gd_import_data" class="btn btn-primary" />
							<input onclick="geodir_location_resume_import(this, 'cat_location')" type="button" value="<?php _e( "Continue Import Data", 'geodirlocation' ); ?>" id="gd_continue_data" class="btn btn-primary" style="display:none"/>
							<input type="button" value="<?php _e("Terminate Import Data", 'geodirlocation');?>" id="gd_stop_import" class="btn btn-danger" name="gd_stop_import" style="display:none" onclick="geodir_location_terminate_import(this, 'cat_location')"/>
		...
		<div id="gd_ie_excat_locations" class="metabox-holder accordion">
			<div class="card p-0 mw-100 border-0 shadow-sm" style="overflow: initial;">
				<div class="card-header bg-white rounded-top">
					<h2	class="gd-settings-title h5 mb-0 "><?php echo __( 'Category + Locations Description: Export CSV', 'geodirlocation' ); ?></h2>
				</div>
				<div id="gd_ie_ex_cat_locations" class=" gd-hndle-pbox card-body">
					<?php
					...
					echo aui()->select(
						array(
							...
							'label'      => __( 'CPT Categories', 'geodirlocation' ),
							'class'      => 'mw-100',
							'wrap_class' => '',
							'id'         => 'gd_post_type',
					...
					<div class="gd-ie-actions d-flex flex-row align-items-center">
						<input data-export="cat_locations" type="submit" value="<?php echo esc_attr( __( 'Export CSV', 'geodirlocation' ) );?>" class="btn btn-primary" name="gd_start_export" id="gd_start_export">
